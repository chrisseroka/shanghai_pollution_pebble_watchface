...
static void init_pollution_update(Window *window) {
  ...
  text_layer_set_text(label_layer, "Shanghai PM2.5:");
  ...
  pm25_value_layer = text_layer_create((GRect) { .origin = { 0, 115 }, .size = { bounds.size.w, bounds.size.h } });
  text_layer_set_text(pm25_value_layer, "Loading...");
  ...
  text_layer_set_font(pm25_value_layer, fonts_get_system_font(FONT_KEY_GOTHIC_18));
  layer_add_child(window_layer, text_layer_get_layer(pm25_value_layer));

  Tuplet initial_values[] = {
     TupletCString(PM25, "Loading...")
  };
